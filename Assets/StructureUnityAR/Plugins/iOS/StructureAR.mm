...
struct Options
{
    ...
    const float initialVolumeResolution = 0.0125; // 4 mm per voxel

    // We will use a color overlay, so we want registered depth.
    const STStreamConfig structureStreamConfig = STStreamConfigRegisteredDepth320x240;
...
#define DEBUG_REMOTE_HOST @"192.168.1.1"
...
-(void) processDepthFrame:(STDepthFrame*)depthFrame colorFrame:(STColorFrame*)colorFrame
{
    ...
    switch (_slamState.scannerState)
    {
        ...
        case ScannerStateViewing:
        {
            ...
            bool trackerGotVeryGoodPose = [_slamState.tracker updateCameraPoseWithDepthFrame:depthFrame colorFrame:colorFrame error:&trackingError];
            
            bool trackerWasTotallyLost = false;
            
            // Integrate it into the current mesh estimate if tracking was very successful.
            if (trackerGotVeryGoodPose)
            {
                if(_slamState.scannerState == ScannerStateScanning) // not integrating into the mesh during the game.
                    [_slamState.mapper integrateDepthFrame:depthFrame cameraPose:[_slamState.tracker lastFrameCameraPose]];
            }
            else
            {
                // Uncomment this for heavier debugging.
                NSLog(@"STTrackerError: %@", [trackingError localizedDescription]);
                
                switch (trackingError.code)
                {
                    case STErrorTrackerLostTrack:
                        trackerWasTotallyLost = true;
                        break;
                        
                    default:
                        // In the other situations the tracker could still estimate a pose, so we'll use it.
                        break;
                }
            }
            
            if (!trackerWasTotallyLost)
            {
            ...
            ucb.notifyTrackingStatus(!trackerWasTotallyLost);
        }
...
- (void)setupSLAM
{
    ...
    NSDictionary* trackerOptions = @{
                                     kSTTrackerTypeKey: _options.useColorTracking ? @(STTrackerDepthAndColorBased) : @(STTrackerDepthBased),
